...
class Pokemon:
    ...
    def add_hp(self):
        if self.element == "草":
            ...
            print(f"{self.name}触发了草元素被动回复了{ self.h p *0.1 }点血,现在的血量为{self.hp}")
    ...
    def attack_move(self, other, n):
        ...
        if self.judge(self):
            ...
            if damage < 0:
                ...
                print(f"{self.name} 对 {other.name} 造成了 {damage} 点伤害")
...
class ElectricPokemon(Pokemon):  # 皮卡丘（电）
    def __init__(self, name, hp, attack, defense):
        ...
        self.status_effects = []  # 当前状态
        self.move = [{"name": "十万伏特", "time": 1.4, "pro": "麻痹效果"},
                     {"name": "电光一闪", "time": 1.0, "pro": "快速攻击"}]
        self.protime = 1
